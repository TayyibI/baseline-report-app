     ...
     function detectJSFeatures(ast, verbose = false) {
       ...
       ast.program.body.forEach((node) => {
         ...
         if (
           ...
           if (node.expression.arguments.length > 1 && node.expression.arguments[1].type === 'ObjectExpression') {
             hasFetchWithInit = true;
             if (verbose) console.log('Detected fetch with init options');
           }
         }
         if (
           node.type === 'ExpressionStatement' &&
       ...
       if (hasFetchWithInit) features.push({ name: 'fetch with init options', key: 'fetch' });
       ...
       root.walkRules((rule) => {
         ...
         rule.walkDecls((decl) => {
           ...
           if (decl.prop === 'container-type' || decl.prop === 'container-name' || decl.prop === 'container') {
             features.push({ name: 'container-queries', key: 'container-queries' });
             if (verbose) console.log('Detected container-queries');
           }
           if (decl.prop.startsWith('scroll-snap')) {
             features.push({ name: 'scroll-snap', key: 'scroll-snap' });
             if (verbose) console.log('Detected scroll-snap');
           }
         });
       });
       return features;
