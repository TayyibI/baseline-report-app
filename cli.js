#!/usr/bin/env node
   ...
   function detectJSFeatures(ast, verbose = false) {
     ...
     const detected = {
       ...
       Temporal: { found: false, line: null },
     };
       ...
       if (node.type === 'NewExpression' && node.callee.name === 'AbortController' && !detected.AbortController.found) {
         ...
         node.declarations.forEach((decl) => {
           ...
           if (
             ...
             decl.init.object.name === 'Temporal' &&
             decl.init.property.name === 'Now' &&
             !detected.Temporal.found
           ) {
             detected.Temporal = { found: true, line: decl.loc?.start?.line || 1 };
             if (verbose) console.log(`Detected Temporal.Now at line ${decl.loc?.start?.line}`);
           }
         });
       }
       if (
     ...
     if (detected.Temporal.found) features.push({ name: 'Temporal.Now', key: 'temporal', line: detected.Temporal.line });
   ...
   function detectCSSFeatures(css, verbose = false, file = '') {
     ...
     const detected = {
       ...
       colorMix: { found: false, line: null },
     };
     ...
     root.walkRules((rule) => {
       ...
       rule.walkDecls((decl) => {
         ...
         if (decl.prop === 'color' && decl.value.includes('color-mix') && !detected.colorMix.found) {
           detected.colorMix = { found: true, line: decl.source?.start?.line || 1 };
           if (verbose) console.log(`Detected color-mix at line ${decl.source?.start?.line}`);
         }
       });
     });
     ...
     if (detected.colorMix.found) features.push({ name: 'color-mix', key: 'color-mix', line: detected.colorMix.line });
   ...
   program
     ...
     .action((folderPath, options) => {
       ...
       try {
         ...
         const bar = new ProgressBar('Scanning [:bar] :percent :current/:total files', {
           total: files.length,
           width: 40,
           complete: '=',
           incomplete: ' ',
         ...
         const results = [];
         files.forEach((file) => {
           if (options.filter === 'js' && !file.endsWith('.js')) return bar.tick();
           if (options.filter === 'css' && !file.endsWith('.css')) return bar.tick();
           ...
           try {
             ...
             filtered.forEach((feat) => {
               ...
               const status = ['nesting', 'subgrid', 'temporal', 'color-mix'].includes(feat.key) ? 'baseline' : (featureData?.status?.baseline === 'high' ? 'baseline' : 'non-baseline');
